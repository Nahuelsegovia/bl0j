			...
			<div class="panel-body">
				...
				<%  excludedProps = Event.allEvents.toList() << 'id' << 'version'
					...
					<div class="row" style="padding-left: 15px;">
						<span style="font-weight: bold;"><g:message code="${domainClass.propertyName}.${p.name}.label" default="${p.naturalName}" /></span>: 
						<%  if (p.isEnum()) { %>
						...
						<%  } else if (p.type == Date || p.type == java.sql.Date || p.type == java.sql.Time || p.type == Calendar) { %>
							<span class="property-value" aria-labelledby="${p.name}-label"><g:formatDate date="\${${propertyName}?.${p.name}}" format="dd/MM/yyyy HH:mm" /></span>
